...
const manifestStr = `
{
  ...
  "description": "客户化定制插件：自定义聊天窗口时间显示格式、新成员历史消息隔离、私信已读/未读标记、批量删除消息，以及在全局头部右上角提供可配置的产品导航。",
  ...
  "settings_schema": {
    "header": "配置 Customers Plugin 的各项能力。时间格式支持管理员统一配置与用户个人偏好；历史消息隔离、私信已读/未读、批量删除与产品导航均为服务端行为，用户端无法绕过。",
    "footer": "",
    "settings": [
      {
        "key": "Enabled",
      ...
      {
        "key": "BulkDeleteEnabled",
        "display_name": "启用批量删除消息",
        "type": "bool",
        "help_text": "开启后，系统管理员可在下方「批量删除消息」面板按条件删除消息，频道管理员也可在会话中勾选消息删除。删除不可恢复，请谨慎开启。",
        "placeholder": "",
        "default": false,
        "hosting": "",
        "secret": false
      ...
      {
        "key": "BulkDeleteMaxPosts",
        "display_name": "单次删除上限",
        "type": "number",
        "help_text": "一次请求最多删除多少条消息，用于防止误操作波及面过大。留空或填 0 表示 500 条，最大 10000 条。",
        "placeholder": "",
        "default": 500,
        "hosting": "",
        "secret": false
      },
      {
        ...
        "display_name": "批量删除消息",
        "type": "custom",
        "help_text": "按会话、发送者、关键词和时间范围筛选消息，先预览数量再执行删除。",
        "placeholder": "",
        "default": null,
        "hosting": "",
        "secret": false
      ...
      {
        "key": "ProductNavEnabled",
        "display_name": "启用产品导航",
        "type": "bool",
        "help_text": "开启后，全局头部右上角会显示「产品导航」按钮，点击展开下方配置的分类与网站链接。未配置任何链接时不显示按钮，因此开启本身不会改变界面。",
        "placeholder": "",
        "default": true,
        "hosting": "",
        "secret": false
      ...
      {
        "key": "ProductNavIconUrl",
        "display_name": "产品导航按钮图标",
        "type": "text",
        "help_text": "按钮上显示的图片地址，例如 https://example.com/static/nav.png 或站点相对路径 /static/nav.png。留空则使用插件内置的九宫格图标。仅接受 http(s) 与站点相对路径，其他值会被忽略。建议 20×20 以上、背景透明。",
        "placeholder": "留空 = 使用内置图标",
        "default": "",
        "hosting": "",
        "secret": false
      },
      {
        "key": "ProductNavLinksPerRow",
        "display_name": "每行显示的链接数",
        "type": "number",
        "help_text": "展开面板后，每个分类一行显示多少个链接，例如填 5 就是一行 5 个，超出部分换到下一行。面板宽度会跟着这个数字自动调整。可填 1 到 12，留空、0 或超出范围则按 5 处理。",
        "placeholder": "",
        "default": 5,
        "hosting": "",
        "secret": false
      },
      {
        ...
        "display_name": "产品导航",
        "type": "custom",
        "help_text": "添加分类，并在每个分类下添加网站链接与 logo 图标。保存后所有用户刷新页面即可在右上角看到。",
        "placeholder": "",
        "default": null,
        "hosting": "",
        "secret": false
      ...
      {
        "key": "HistoryPendingPolicy",
        "display_name": "边界未取回时消息行的处理",
        "type": "dropdown",
        "help_text": "切换频道时服务端可能还没返回该频道的可见边界。「留白」会隐藏行内容但保留行高（不会泄露历史，也不会让滚动条跳动）；「照常显示」会先渲染出来，可能有一瞬间能看到加入前的消息。",
        "placeholder": "",
        "default": "blank",
        "options": [
          {
