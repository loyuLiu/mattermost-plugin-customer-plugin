...
# The build tools always have to run on the host machine, therefore we must override any
# GOOS/GOARCH persisted through `go env -w` (a very common setup when the target server
# runs on Linux while development happens on macOS / Windows).
HOST_GOOS ?= $(shell go env GOHOSTOS)
HOST_GOARCH ?= $(shell go env GOHOSTARCH)

# Ensure that the build tools are compiled. Go's caching makes this quick.
$(shell cd build/manifest && env GOOS=$(HOST_GOOS) GOARCH=$(HOST_GOARCH) $(GO) build -ldflags '-X "main.BuildHashShort=$(BUILD_HASH_SHORT)" -X "main.BuildTagLatest=$(BUILD_TAG_LATEST)" -X "main.BuildTagCurrent=$(BUILD_TAG_CURRENT)"' -o ../bin/manifest)

# Ensure that the deployment tools are compiled. Go's caching makes this quick.
$(shell cd build/pluginctl && env GOOS=$(HOST_GOOS) GOARCH=$(HOST_GOARCH) $(GO) build -o ../bin/pluginctl)
